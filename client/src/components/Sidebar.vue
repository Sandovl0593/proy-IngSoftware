...
export default {
    ...
    data() {
      return {
        ...
        miembrosNoAtendidos: ["Margiory", "Marcela", "Milloshy", "Fabiola", "Adrian"],
        // miembrosNoAtendidos: []
      };
    },
    methods: {
    ...
    <ul class="sidenav-nav">
        <li class="sidenav-nav-item">
          <!-- si esta en mismo /dashboard, no hacer nada -->
           <router-link class="sidenav-nav-link" :to="`/dashboard/${$props.tid}/${$props.code}/${$props.role}`" exact>
...
export default {
  ...
  setup(props) {
    ...
    const obtenerMiembrosNoAtendidos = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:8000/obtener_miembros_no_atendidos/${props.tid}`);
